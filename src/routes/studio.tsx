  ...
  return (
    <div className="producer-page producer-dashboard-page">
      ...
      <div className="dashboard-grid-secondary">
        ...
        <section className="surface-card dashboard-card"><PanelHeader title="Workflow" description="Status pekerjaan editorial." /><div className="workflow-list"><WorkflowRow label="Draft" value={drafts} tone="blue" /><WorkflowRow label="Published" value={published} tone="blue-strong" /><WorkflowRow label="Archived" value={archived} tone="gray" /></div></section>
      </div>
    </div>
  );
}
...
function AnalisisStudio({ token, items, onChange, busy, setBusy, selectId }: { token: string; items: AnalisisItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void; selectId?: number | null }) {
  ...
  async function save(status: AnalisisStatus) {
    setError("");
    setBusy(true);
    try { await saveAnalisis({ data: { token, ...form, status } }); setForm((prev) => ({ ...prev, status })); await onChange(); }
  ...
  async function remove() {
    ...
    setBusy(true);
    try { await deleteAnalisis({ data: { token, id: form.id } }); setForm(emptyAnalisis()); await onChange(); }
  ...
  return (
    <div className="producer-page editor-page">
      <PageHeader eyebrow="Analisis CMS" title="Analisis" description="Kelola insight market dengan struktur yang rapi, cepat diedit, dan siap dipublikasikan."><button type="button" className="btn btn-primary" onClick={() => { setForm(emptyAnalisis()); setSection("overview"); }}><Plus size={16} /> Analisis Baru</button></PageHeader>
      <div className="editor-layout">
        <aside className="surface-card content-library">
          <div className="content-library-head"><div><span className="panel-eyebrow">Content library</span><h2>Semua Analisis</h2><p>{items.length} konten</p></div><button type="button" className="icon-btn subtle-icon" onClick={() => setForm(emptyAnalisis())} title="Analisis baru"><Plus size={17} /></button></div>
        ...
        <section className="editor-workspace">
          <form onSubmit={(e) => { e.preventDefault(); void save(form.status); }} className="surface-card editor-surface">
            ...
            <div className="editor-action-bar"><div className="editor-action-left">{form.id ? <button type="button" className="btn btn-danger-outline" onClick={() => void remove()} disabled={busy}><Trash2 size={15} /> Hapus</button> : <span className="save-state"><span className="save-dot" /> Perubahan lokal belum disimpan</span>}</div><div className="editor-action-right"><button type="button" className="btn btn-outline" onClick={() => { setForm(emptyAnalisis()); setSection("overview"); }}>Batal</button><button type="button" className="btn btn-outline" onClick={() => void save("DRAFT")} disabled={busy}>{busy ? "Menyimpan…" : "Simpan Draft"}</button><button type="button" className="btn btn-primary" onClick={() => void save("PUBLISHED")} disabled={busy}><Check size={15} /> {busy ? "Memproses…" : form.id && form.status === "PUBLISHED" ? "Update" : "Publish"}</button></div></div>
          </form>
        </section>
...
function NewsStudio({ token, items, onChange, busy, setBusy, selectId }: { token: string; items: NewsItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void; selectId?: number | null }) {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); setError(""); setBusy(true); try { await saveNews({ data: { token, ...form } }); await onChange(); } catch (err) { setError(err instanceof Error ? err.message : "Gagal menyimpan."); } finally { setBusy(false); } }
  async function remove() { if (!form.id || !confirm("Hapus news ini?")) return; setBusy(true); try { await deleteNews({ data: { token, id: form.id } }); setForm(emptyNews()); await onChange(); } catch (err) { setError(err instanceof Error ? err.message : "Gagal menghapus."); } finally { setBusy(false); } }
...
function EdukasiStudio({ token, items, onChange, busy, setBusy, selectId }: { token: string; items: EdukasiItem[]; onChange: () => Promise<void>; busy: boolean; setBusy: (v: boolean) => void; selectId?: number | null }) {
  ...
  const [query, setQuery] = useState("");
  const [levelFilter, setLevelFilter] = useState<"ALL" | EduLevel>("ALL");
  const [section, setSection] = useState<EducationSection>("overview");
  const [error, setError] = useState("");
  const filtered = useMemo(() => items.filter((item) => {
    const matchesQuery = `${item.title} ${item.level}`.toLowerCase().includes(query.trim().toLowerCase());
