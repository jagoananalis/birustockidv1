...
export type AnalisisStatus = "DRAFT" | "PUBLISHED" | "ARCHIVED";
...
const STATUSES: AnalisisStatus[] = ["DRAFT", "PUBLISHED", "ARCHIVED"];

function asAnalisisStatus(value: string): AnalisisStatus {
  return STATUSES.includes(value as AnalisisStatus) ? (value as AnalisisStatus) : "PUBLISHED";
}
...
function mapAnalisis(row: AnalisisRow): AnalisisItem {
  return {
    ...
    status: asAnalisisStatus(row.status),
  };
}
...
const analisisInput = z.object({
  ...
  excerpt: z.string().min(8).refine((value) => value.trim().split(/\s+/).filter(Boolean).length <= 600, "Ringkasan analisis maksimal 600 kata"),
...
export const listAnalisis = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const rows = await sql<AnalisisRow>`
    select id, slug, pair, title, excerpt, body, image_url, accent, published_at, timeframe, bias, support, resistance, target, invalidation, scenario_bullish, scenario_bearish, updated_at, status
    from analisis
...
export const listAnalisisStudio = createServerFn({ method: "GET" })
...
export const saveAnalisis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const inserted = await sql<{ id: number }>`
      insert into analisis (slug, pair, title, excerpt, body, image_url, accent, published_at, timeframe, bias, support, resistance, target, invalidation, scenario_bullish, scenario_bearish, status)
      values (
        ${slug},
        ${data.pair.trim()},
        ${data.title.trim()},
