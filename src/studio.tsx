...
function PreviewImage({ src, alt }: { src: string; alt: string }) {
  ...
  return <>
    <button type="button" className="preview-image-button" onClick={() => setOpen(true)} aria-label="Buka gambar untuk melihat lebih besar">
      <img src={src} alt="" />
      <span className="preview-image-zoom-hint"><Maximize2 size={14} /> Klik untuk memperbesar</span>
    </button>
...
function ImageLightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  ...
  return <div className="image-lightbox" role="dialog" aria-modal="true" aria-label="Gambar analisis diperbesar" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="image-lightbox-panel">
      <div className="image-lightbox-toolbar">
        <span>Preview gambar</span>
        <div className="image-lightbox-actions">
          <button type="button" className="icon-btn tiny" onClick={() => setScale((v) => Math.max(0.6, +(v - 0.2).toFixed(1)))} title="Zoom out"><ZoomOut size={16} /></button>
          <button type="button" className="zoom-reset" onClick={() => setScale(1)} title="Reset zoom"><RotateCcw size={14} /> {Math.round(scale * 100)}%</button>
          <button type="button" className="icon-btn tiny" onClick={() => setScale((v) => Math.min(3, +(v + 0.2).toFixed(1)))} title="Zoom in"><ZoomIn size={16} /></button>
          <button type="button" className="icon-btn tiny" onClick={onClose} title="Tutup"><X size={16} /></button>
        </div>
      </div>
      <div className="image-lightbox-viewport">
        <img src={src} alt={alt} style={{ transform: `scale(${scale})` }} />
      </div>
      <div className="image-lightbox-hint">Gunakan Zoom +/- · klik di luar gambar atau tekan Esc untuk menutup</div>
    </div>
  </div>;
}
